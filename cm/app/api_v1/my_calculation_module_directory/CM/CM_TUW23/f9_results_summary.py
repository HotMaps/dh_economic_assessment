    ...
    if numLabels < 1:
        ...
        if not term_cond:
            ...
            summary = [{"unit": "MWh", "name": "Total demand in selected region in the first year of investment", "value": float(heat_dem_1st)},
                       ...
                       {"unit": "EUR/yr", "name": "Total grid costs - annuity", "value": float(dist_inv + trans_inv)},
                       {"unit": "EUR/yr", "name": "Total distribution grid costs - annuity", "value": float(dist_inv)},
                       {"unit": "EUR/yr", "name": "Total transmission grid costs - annuity", "value": float(trans_inv)},
